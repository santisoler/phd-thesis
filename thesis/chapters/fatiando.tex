...
gravitatorios de teseroides mediante la \ac{GLQ}.
...
A su vez, la clase \texttt{Ellipsoid} posee una serie de propiedades que se
...
\lstinputlisting[
    ...
    language=Python,
    caption={%
        Ejemplo de cálculo de gravedad normal y conversión de unidades con
        elipsoide de referencia WGS84 mediante Boule (\BouleVersion{})
    },
...
Podemos calcular los campos gravitatorios producidos por masas puntuales
mediante la función \texttt{point\_gravity()}.
El Código~\ref{lst:harmonica-point} ejemplifica cómo podemos utilizar esta
...
calcular los campos gravitatorios de prismas rectangulares mediante la
...
gravitatorios  de teseroides con densidad constante, a los cuales podemos
...
\lstinputlisting[
    ...
    label=lst:harmonica-point
]{fatiando-examples/harmonica-point.py}

Además, desde la versión
\href{https://www.fatiando.org/harmonica/v0.4.0/}{v0.4.0} es posible calcular
los campos gravitatorios de teseroides con densidad variable en la dirección
radial, aplicando la metodología descripta en el Capítulo
\ref{cha:tesseroids-variable-density}.
...
\lstinputlisting[
    ...
    language=Python,
    caption={%
        Ejemplo de cálculo de aceleración de la gravedad producida por un
        teseroide con densidad lineal en la dirección radial utilizando
        Harmonica (\HarmonicaVersion{}).
    },
    label=lst:harmonica-tesseroid-variable-density
]{fatiando-examples/harmonica_tesseroid_variable_density.py}
...
Desde la versión
\href{https://www.fatiando.org/harmonica/v0.4.0/}{v0.4.0}, la clase
\texttt{EquivalentSources} cuenta con la opción de utilizar fuentes promediadas
por bloques, definiendo el tamaño de cada bloque mediante el parámetro
\texttt{block\_size}.
Además, se agregó una nueva clase: \texttt{EquivalentSourcesGB} la cual
implementa las fuentes equivalentes potenciadas por gradiente.
El Código~\ref{lst:harmonica-gradient-boosted} muestra un ejemplo de uso de
esta clase para grillar un conjunto de datos gravimétricos de Sudáfrica
haciendo uso de fuentes promediadas por bloque en conjunto con la potenciación
del gradiente.
...
\lstinputlisting[
    ...
    language=Python,
    caption={%
        Ejemplo de uso de las fuentes potenciadas por gradiente mediante la
        clase \texttt{EquivalentSourcesGB} de Harmonica (\HarmonicaVersion{}).
    },
    label=lst:harmonica-gradient-boosted
]{fatiando-examples/harmonica_gradient_boosted.py}
...
obtener reportes detallados sobre la ejecución de las funciones de prueba,
...
utilizados transversalmente en cada función de prueba, ahorrando la necesidad
de repetir código.
...
\section{Contribuciones del autor al proyecto}
...
Mis contribuciones al proyecto no se limitan únicamente al agregado de nuevas
...
\section{Relación con investigaciones científicas}
...
Existe además una relación de retroalimentación entre el proyecto y estas
...
Un ejemplo de esto son los algoritmos desarrollados a lo largo de esta Tesis
y descriptos en los Capítulos~\ref{cha:tesseroids-variable-density}
y \ref{cha:eql-gradient-boosted}, los cuales fueron incorporados a Harmonica.
...
Además, deseamos incluir cálculos de campos gravitatorios generados por los
...
Por su parte, en Harmonica deseamos implementar algunas de las metodologías
basadas en Transformadas de Fourier, tales como continuación ascendente,
derivadas direccionales, reducción al polo de datos magnéticos o deconvolución
de Euler.
A largo plazo poseemos el objetivo de diseñar un \emph{framework} de inversión
similar al que existía en \texttt{fatiando} v0.5, pero haciendo uso a las
últimas herramientas disponibles.
